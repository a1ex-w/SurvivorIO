...
var objects := {
	# weight:     spawn frequency relative to other objects (higher = more common).
	#             Omitting defaults to 1. No code changes needed for new objects.
	# min_count:  guaranteed minimum always present on the map. Spawner fills
	#             deficient types before doing weighted random selection.
	#             Omitting defaults to 0 (no minimum enforced).
	...
	"ore1":       {"id": "ore1",       "hp": 120, "tool": "pickaxe",  "weight": 4,  "min_count": 2, "drops": {"iron":        {"min": 1, "max": 3}}},
	"magicPlant1":{"id": "magicPlant1","hp": 30,  "tool": "sword",    "weight": 3,  "min_count": 1, "drops": {"magicHerb":   {"min": 1, "max": 2}}},
	"magicTree1": {"id": "magicTree1", "hp": 70,  "tool": "axe",      "weight": 2,  "min_count": 1, "drops": {"magicWood":   {"min": 1, "max": 3}}},
	"magicRock1": {"id": "magicRock1", "hp": 110, "tool": "pickaxe",  "weight": 2,  "min_count": 1, "drops": {"magicStone":  {"min": 1, "max": 2}}},
	"crystal1":   {"id": "crystal1",   "hp": 150, "tool": "pickaxe",  "weight": 1,  "min_count": 1, "drops": {"crystalShard":{"min": 1, "max": 2}}},
}
